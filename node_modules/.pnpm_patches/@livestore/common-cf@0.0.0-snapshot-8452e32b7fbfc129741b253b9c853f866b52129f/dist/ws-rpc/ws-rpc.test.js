...
import { TestRpcs } from "./test-fixtures/rpc-schema.js";
Vitest.describe('Durable Object WebSocket RPC', { timeout: 5000 }, () => {
    const port = process.env.LIVESTORE_WS_PORT;
    const ProtocolLive = RpcClient.layerProtocolSocket().pipe(Layer.provide(Socket.layerWebSocket(`ws://localhost:${port}`)), Layer.provide(Socket.layerWebSocketConstructorGlobal), Layer.provide(RpcSerialization.layerJson));
    // Direct HTTP RPC client tests
    Vitest.scopedLive('should call ping method', Effect.fn(function* () {
    ...
    Vitest.scopedLive('should call echo method', Effect.fn(function* () {
        const client = yield* RpcClient.make(TestRpcs);
        const result = yield* client.Echo({ text: 'Echo' });
        expect(result).toEqual({ echo: 'Echo: Echo' });
    }, Effect.provide(ProtocolLive)));
    Vitest.scopedLive('should call add method', Effect.fn(function* () {
        const client = yield* RpcClient.make(TestRpcs);
        const result = yield* client.Add({ a: 15, b: 25 });
        expect(result).toEqual({ result: 40 });
    }, Effect.provide(ProtocolLive)));
    Vitest.scopedLive('should handle RPC fail method', Effect.fn(function* () {
        const client = yield* RpcClient.make(TestRpcs);
    ...
    Vitest.scopedLive('should handle defect method', Effect.fn(function* () {
        const client = yield* RpcClient.make(TestRpcs);
    ...
    Vitest.scopedLive('should handle streaming RPC via HTTP', Effect.fn(function* () {
        const client = yield* RpcClient.make(TestRpcs);
        const stream = client.Stream({}).pipe(Stream.take(4), Stream.map((c) => c.maybeNumber.pipe(Option.getOrUndefined)));
        const chunks = yield* Stream.runCollect(stream);
        expect(Chunk.toReadonlyArray(chunks)).toEqual([1, 4, 9, 16]); // squares of 1,2,3,4
    }, Effect.provide(ProtocolLive)));
    Vitest.scopedLive('should handle streaming RPC with error via HTTP', Effect.fn(function* () {
        const client = yield* RpcClient.make(TestRpcs);
    ...
    Vitest.scopedLive('should handle streaming RPC with defect via HTTP', Effect.fn(function* () {
        const client = yield* RpcClient.make(TestRpcs);
    ...
    Vitest.scopedLive('should handle stream interruption via HTTP', Effect.fn(function* () {
        const client = yield* RpcClient.make(TestRpcs);
...
Vitest.describe('Hibernation Tests', { timeout: 25000 }, () => {
    const port = process.env.LIVESTORE_WS_PORT;
    const ProtocolLive = RpcClient.layerProtocolSocket().pipe(Layer.provide(Socket.layerWebSocket(`ws://localhost:${port}`)), Layer.provide(Socket.layerWebSocketConstructorGlobal), Layer.provide(RpcSerialization.layerJson));
    Vitest.scopedLive('should maintain RPC functionality after hibernation', Effect.fn(function* () {
        ...
        const stream = client.Stream({}).pipe(Stream.take(3), Stream.map((c) => c.maybeNumber.pipe(Option.getOrUndefined)));
        const chunks = yield* Stream.runCollect(stream);
    ...
    Vitest.scopedLive('should handle rapid operations after hibernation', Effect.fn(function* () {
        ...
        const operations = Array.from({ length: 5 }, (_, i) => Effect.gen(function* () {
        ...
        console.log('✅ All rapid operations successful:', results.map((r) => r.operation));
        console.log('🎉 Rapid operations work correctly after hibernation!');
    }, Effect.provide(ProtocolLive)));
});
//# sourceMappingURL=ws-rpc.test.js.map