...
import { Effect, HttpApp, Layer, Option, RpcClient, RpcSerialization, RpcServer, Schedule, Stream, } from '@livestore/utils/effect';
...
export class TestRpcDurableObject extends DurableObject {
    ...
    async rpc(payload) {
        const TestRpcsLive = TestRpcs.toLayer({
            ...
            Stream: () => Stream.iterate(1, (n) => n + 1).pipe(Stream.map((n) => ({ maybeNumber: Option.some(n * n) })), // Stream squares: 1, 4, 9, 16, ...
            ...
            Stream.take(100)),
            StreamError: ({ count, errorAfter }) => Stream.range(1, count).pipe(Stream.map((n) => n * n), Stream.mapEffect((n) => n > errorAfter ? Effect.fail(`Stream error after ${errorAfter}: got ${n}`) : Effect.succeed(n))),
            StreamDefect: ({ count, defectAfter }) => Stream.range(1, count).pipe(Stream.map((n) => n * n), Stream.mapEffect((n) => n > defectAfter ? Effect.die(`Stream defect after ${defectAfter}: got ${n}`) : Effect.succeed(n))),
            StreamInterruptible: ({ delay }) => Stream.iterate(1, (n) => n + 1).pipe(Stream.map((n) => n), Stream.schedule(Schedule.spaced(delay))),
        });
        const result = await toDurableObjectHandler(TestRpcs, { layer: TestRpcsLive })(payload).pipe(Effect.tapCauseLogPretty, Effect.runPromise);
        return result;
    }
}
export default {
            ...
            if (url.pathname === '/rpc') {
                ...
                return Effect.gen(function* () {
                    ...
                    const handlersLayer = TestRpcs.toLayer({
                        ...
                        StreamInterruptible: (msg) => doRpcClient.StreamInterruptible(msg).pipe(Stream.take(msg.interruptAfterCount), Stream.orDie),
                    }).pipe(Layer.provideMerge(RpcServer.layerProtocolHttp({ path: '/rpc' })), Layer.provideMerge(RpcSerialization.layerJson));
