...
export const setupDurableObjectWebSocketRpc = ({ doSelf, rpcLayer, webSocketMode, onMessage, mainLayer, }) => {
    ...
    const launchServer = (ws) => Effect.gen(function* () {
        ...
        const ctx = {
            scope,
            onMessage: (message) => incomingQueue
                .offer(message)
                .pipe(Effect.asVoid, Effect.withSpan('ws-rpc-server/onMessage', { root: true }), Effect.provide(runtime), Effect.runPromise),
        };
        serverCtxMap.set(ws, ctx);
        return ctx;
    }).pipe(Effect.tapCauseLogPretty, Logger.withMinimumLogLevel(LogLevel.Debug), // Useful for debugging
    Effect.provide(Layer.mergeAll(Logger.consoleWithThread('ws-rpc-server'), mainLayer ?? Layer.empty)), Effect.withSpan('effect-ws-rpc-server'), Effect.runPromise);
    const webSocketMessage = async (ws, message) => {
 ...
 */
const makeSocketProtocol = ({ incomingQueue, ws, onMessage }) => Effect.gen(function* () {
    const serialization = yield* RpcSerialization.RpcSerialization;
    const disconnects = yield* Mailbox.make();
    const writeRaw = (msg) => Effect.succeed(ws.send(msg));
    let writeRequest;
    ...
    const protocol = yield* RpcServer.Protocol.make((writeRequest_) => {
        ...
        const startProcessing = Mailbox.toStream(incomingQueue).pipe(Stream.tap((data) => {
            try {
        ...
        }), Stream.runDrain, Effect.tapCauseLogPretty, Effect.fork);
