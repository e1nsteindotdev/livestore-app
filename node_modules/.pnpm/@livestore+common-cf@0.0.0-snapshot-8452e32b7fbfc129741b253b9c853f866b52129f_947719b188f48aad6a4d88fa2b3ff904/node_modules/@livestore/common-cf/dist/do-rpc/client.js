import { Effect, Fiber, Layer, RpcClient, RpcSerialization } from '@livestore/utils/effect';
/**
...
const processReadableStream = (stream, parser, writeResponse) => Effect.gen(function* () {
    ...
    }).pipe(Effect.ensuring(Effect.promise(() => reader.cancel()).pipe(Effect.andThen(() => Effect.sync(() => reader.releaseLock())))));
});
/**
 ...
 */
export const layerProtocolDurableObject = (args) => Layer.scoped(RpcClient.Protocol, makeProtocolDurableObject(args));
/**
    ...
    const send = (message) => {
        ...
        return Effect.gen(function* () {
            ...
            if (serializedResponse instanceof ReadableStream) {
                const fiber = yield* processReadableStream(serializedResponse, parser, writeResponse).pipe(Effect.fork);
