...
import { BucketQueue, Effect, Exit, FiberHandle, Option, Queue, Schema, Stream, Subscribable, } from '@livestore/utils/effect';
import { SyncError, } from "../adapter-types.js";
import * as EventSequenceNumber from "../schema/EventSequenceNumber.js";
import * as LiveStoreEvent from "../schema/LiveStoreEvent.js";
import { getEventDef } from "../schema/mod.js";
...
export const makeClientSessionSyncProcessor = ({ schema, clientSession, runtime, materializeEvent, rollback, refreshTables, span, params, confirmUnsavedChanges, }) => {
    ...
    const simSleep = (key, key2) => Effect.sleep((params.simulation?.[key]?.[key2] ?? 0));
    const syncStateRef = {
    ...
    const push = Effect.fn('client-session-sync-processor:push')(function* (batch) {
        // TODO validate batch
    ...
    const boot = Effect.gen(function* () {
        ...
        yield* Stream.suspend(() => clientSession.leaderThread.events.pull({ cursor: syncStateRef.current.upstreamHead })).pipe(Stream.tap(({ payload }) => Effect.gen(function* () {
            ...
            if (mergeResult._tag === 'rebase') {
                ...
                if (LS_DEV) {
                    yield* Effect.logDebug('merge:pull:rebase: rollback', mergeResult.rollbackEvents.length, ...mergeResult.rollbackEvents.slice(0, 10).map((_) => _.toJSON()));
                }
        ...
        }).pipe(Effect.tapCauseLogPretty, Effect.catchAllCause((cause) => clientSession.shutdown(Exit.failCause(cause))))), Stream.runDrain, Effect.forever, // NOTE Whenever the leader changes, we need to re-start the stream
