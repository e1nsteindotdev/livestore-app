...
export const bootDevtools = (options) => Effect.gen(function* () {
    ...
    yield* node.listenForChannel.pipe(Stream.filter((res) => Devtools.isChannelName.devtoolsClientLeader(res.channelName, { storeId, clientId }) && res.mode === mode), Stream.tap(({ channelName, source }) => Effect.gen(function* () {
        ...
        yield* syncProcessor.pull({ cursor: syncState.localHead }).pipe(Stream.tap(({ payload }) => sendMessage(Devtools.Leader.SyncPull.make({ payload, liveStoreVersion }))), Stream.runDrain, Effect.forkScoped);
        yield* listenToDevtools({
        ...
        switch (decodedEvent._tag) {
            ...
            case 'LSD.Leader.SyncHistorySubscribe': {
                ...
                if (syncBackend !== undefined) {
                    ...
                    yield* syncBackend.pull(Option.none(), { live: true }).pipe(Stream.map((_) => _.batch), Stream.flattenIterables, Stream.tap(({ eventEncoded, metadata }) => sendMessage(Devtools.Leader.SyncHistoryRes.make({
                        eventEncoded,
                        metadata,
                        subscriptionId,
                        ...reqPayload,
