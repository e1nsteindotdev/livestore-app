...
import { encodeOutgoingMessage, } from "./shared.js";
export const makePush = ({ storage, requestId, options, rpcSubscriptions, pushSemaphore, currentHeadRef, storeId, payload, ctx, respond, }) => (decodedMessage) => Effect.gen(function* () {
    yield* Effect.log(`Pushing ${decodedMessage.batch.length} events`, decodedMessage.batch);
    const respondPush = (message) => Effect.gen(function* () {
    ...
    if (options?.onPush) {
        yield* Effect.tryAll(() => options.onPush(decodedMessage, { storeId, payload })).pipe(UnexpectedError.mapToUnexpectedError);
    }
    ...
    if (firstEvent.parentSeqNum !== currentHeadRef.current) {
        ...
        yield* Effect.log(`Invalid parent event number. Received e${firstEvent.parentSeqNum} but expected e${currentHeadRef.current}`);
        yield* respondPush(SyncMessage.SyncError.make({
