...
export const makePull = ({ storage }) => (req) => Effect.gen(function* () {
    // TODO use streaming
    ...
    //       ([[]] as never)
    //     :
    const batches = Array.from({ length: Math.ceil(remainingEvents.length / PULL_CHUNK_SIZE) }, (_, i) => remainingEvents.slice(i * PULL_CHUNK_SIZE, (i + 1) * PULL_CHUNK_SIZE)).map((batch, i) => SyncMessage.PullResponse.make({
        batch,
