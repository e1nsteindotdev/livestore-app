 ...
 */
export const createHttpRpcHandler = (options) => Effect.gen(function* () {
    ...
    return yield* Effect.promise(() => webHandler(options.request)).pipe(Effect.timeout(10000));
}).pipe(Effect.withSpan('createHttpRpcHandler'));
const createHttpRpcLayer = (options) => SyncHttpRpc.toLayer({
    'SyncHttpRpc.Pull': (req) => Effect.gen(function* () {
        const pull = makePull({
        ...
        return yield* pull(req).pipe(Stream.runHead, Effect.flatten);
    }).pipe(Effect.mapError((e) => SyncMessage.SyncError.make({
    ...
    'SyncHttpRpc.Ping': (req) => Effect.gen(function* () {
        // if (!options.onPing) {
