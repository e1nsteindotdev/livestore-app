...
export const makeCfSync = (options) => ({ storeId, payload }) => Effect.gen(function* () {
    ...
    return SyncBackend.of({
        ...
        pull: (args) => Effect.gen(function* () {
            ...
            return Stream.fromPubSub(incomingMessages).pipe(Stream.tap((_) => _._tag === 'SyncMessage.SyncError' && _.requestId === requestId
        ...
        push: (batch) => Effect.gen(function* () {
            ...
            const requestId = nanoid();
            yield* Stream.fromPubSub(incomingMessages).pipe(Stream.tap((_) => _._tag === 'SyncMessage.SyncError' && _.requestId === requestId
                ? Deferred.fail(pushAck, new InvalidPushError({ reason: { _tag: 'Unexpected', cause: _ } }))
                : Effect.void), Stream.filter((_) => _._tag === 'SyncMessage.PushAck' && _.requestId === requestId), Stream.take(1), Stream.tap(() => Deferred.succeed(pushAck, void 0)), Stream.runDrain, Effect.tapCauseLogPretty, Effect.fork);
...
const connect = (wsUrl, webSocketFactory) => Effect.gen(function* () {
    ...
    const send = (message) => Effect.gen(function* () {
        ...
        if (LS_DEV) {
            yield* Effect.spanEvent(`Sent message: ${message._tag}`, message._tag === 'SyncMessage.PushRequest'
                ? {
    ...
    const innerConnect = Effect.gen(function* () {
        ...
        yield* Effect.eventListener(socket, 'message', (event) => Effect.gen(function* () {
            const decodedEventRes = Schema.decodeUnknownEither(Schema.parseJson(SyncMessage.BackendToClientMessage))(event.data);
