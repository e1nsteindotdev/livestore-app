...
import { Effect, HttpClient, HttpClientRequest, Layer, Option, RpcClient, RpcSerialization, Schedule, Stream, SubscriptionRef, } from '@livestore/utils/effect';
...
export const makeHttpSync = (options) => ({ storeId, payload }) => Effect.gen(function* () {
    ...
    const ping = Effect.gen(function* () {
        ...
        if (res.requestId !== requestId) {
            return shouldNeverHappen(`Ping response requestId ${res.requestId} does not match expected requestId ${requestId}`);
        }
    ...
    const pull = (args) => {
        ...
        // })),
        Stream.mapError((cause) => new InvalidPullError({ cause })), Stream.withSpan('http-sync-client:pull'));
    };
