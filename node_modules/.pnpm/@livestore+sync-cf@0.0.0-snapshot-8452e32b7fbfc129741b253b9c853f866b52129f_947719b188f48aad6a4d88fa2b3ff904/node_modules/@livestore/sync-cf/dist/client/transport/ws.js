...
export const makeWsSync = (options) => ({ storeId, payload }) => Effect.gen(function* () {
    ...
    return SyncBackend.of({
        isConnected,
        connect: ping,
        pull: (args, options) => rpcClient.SyncWsRpc.Pull({
            storeId,
            payload,
            cursor: Option.getOrUndefined(args)?.cursor,
            live: options?.live ?? false,
        }).pipe(Stream.mapError((cause) => cause._tag === 'RpcClientError' && Socket.isSocketError(cause.cause)
            ? new IsOfflineError({ cause: cause.cause })
            : new InvalidPullError({ cause })), Stream.withSpan('pull')),
        push: (batch) => Effect.gen(function* () {
            if (batch.length === 0) {
                return;
            }
            return yield* rpcClient.SyncWsRpc.Push({ storeId, payload, batch }).pipe(Effect.mapError((cause) => new InvalidPushError({
                reason: cause._tag === 'SyncMessage.SyncError' &&
