...
export const makeHttpSync = (options) => ({ storeId, payload }) => Effect.gen(function* () {
    ...
    const pull = (args, options) => Effect.gen(function* () {
        ...
        return rpcClient.SyncHttpRpc.Pull({ storeId, payload, cursor }).pipe(Stream.emitIfEmpty(SyncBackend.pullResItemEmpty()), options?.live
            ? // Phase 2: Simulate `live` pull by polling for new events
                Stream.concatWithLastElement((lastElement) => {
                    const initialPhase2Cursor = lastElement.pipe(Option.flatMap((_) => Option.fromNullable(_.batch.at(-1)?.eventEncoded.seqNum)), Option.getOrElse(() => cursor));
                    return Stream.unfoldChunkEffect(initialPhase2Cursor, (currentCursor) => Effect.gen(function* () {
                        ...
                        const nextCursor = Chunk.last(items).pipe(Option.map((item) => item.batch.at(-1)?.eventEncoded.seqNum), Option.getOrElse(() => currentCursor));
                        return Option.some([items, nextCursor]);
                    }));
                })
            : identity);
    }).pipe(Stream.unwrapScoped, Stream.mapError((cause) => new InvalidPullError({ cause })), Stream.withSpan('http-sync-client:pull'));
    ...
    }).pipe(pushSemaphore.withPermits(1), Effect.mapError((error) => new InvalidPushError({ reason: { _tag: 'Unexpected', cause: error } })), Effect.withSpan('http-sync-client:push'));
