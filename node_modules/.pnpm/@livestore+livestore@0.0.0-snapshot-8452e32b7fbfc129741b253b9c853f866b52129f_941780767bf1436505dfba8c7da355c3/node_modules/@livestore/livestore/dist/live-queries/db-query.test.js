...
import { events, makeTodoMvc, tables } from "../utils/tests/fixture.js";
import { getAllSimplifiedRootSpans, getSimplifiedRootSpan } from "../utils/tests/otel.js";
...
Vitest.describe('otel', () => {
    ...
    Vitest.scopedLive('with thunks with query builder and without labels', () => Effect.gen(function* () {
        ...
        const query$ = queryDb((get) => tables.todos.where(get(filter)).first({ behaviour: 'fallback', fallback: () => defaultTodo }));
        expect(store.query(query$)).toMatchInlineSnapshot(`
      {
        "completed": false,
        "id": "",
    ...
    Vitest.scopedLive('QueryBuilder subscription - basic functionality', () => Effect.gen(function* () {
        const { store, exporter, span, provider } = yield* makeQuery;
    ...
    Vitest.scopedLive('QueryBuilder subscription - unsubscribe functionality', () => Effect.gen(function* () {
        const { store, exporter, span, provider } = yield* makeQuery;
        const callbackResults1 = [];
        const callbackResults2 = [];
        const defaultTodo = { id: '', text: '', completed: false };
        const queryBuilder = tables.todos
            .where({ completed: false })
            .first({ behaviour: 'fallback', fallback: () => defaultTodo });
    ...
    Vitest.scopedLive('QueryBuilder subscription - direct table subscription', () => Effect.gen(function* () {
        const { store, exporter, span, provider } = yield* makeQuery;
        const callbackResults = [];
