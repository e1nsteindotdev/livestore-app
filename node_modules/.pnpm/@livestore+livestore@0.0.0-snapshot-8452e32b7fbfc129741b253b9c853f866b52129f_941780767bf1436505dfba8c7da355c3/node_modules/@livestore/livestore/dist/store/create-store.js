...
export const createStore = ({ schema, adapter, storeId, context = {}, boot, batchUpdates, disableDevtools, onBootStatus, shutdownDeferred, params, debug, confirmUnsavedChanges = true, syncPayload, }) => Effect.gen(function* () {
    ...
    return yield* Effect.gen(function* () {
        ...
        const runtime = yield* Effect.runtime();
        const shutdown = (exit) => Effect.gen(function* () {
            yield* Scope.close(lifetimeScope, exit).pipe(Effect.logWarnIfTakesLongerThan({ label: '@livestore/livestore:shutdown', duration: 500 }), Effect.timeout(1000), Effect.catchTag('TimeoutException', () => Effect.logError('@livestore/livestore:shutdown: Timed out after 1 second')));
            if (shutdownDeferred) {
